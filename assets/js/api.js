 ...
 *
 * This is the ONLY module permitted to read window.DEMO_DATA. Every page,
 * every panel, every widget goes through the functions below. Each one is
 * async and shaped like the FastAPI route it will eventually call, so step 5
 * replaces the body of each function with a fetch() and nothing else moves.
 *
 * The route is written above each function. Argument names mirror the path
 * and query parameters of that route.
 *
 * STATE. The site is static and multi-page, so anything held only in memory is
 * gone the moment the user clicks a link — which would make recording a payment
 * on invoices.html and then opening defaulters.html a lie. The store hydrates
 * from sessionStorage['shule.store'] when it is there and seeds from
 * DEMO_DATA when it is not, and every write persists before it resolves.
 *
 * THE LEDGER. Every movement of money posts a debit and a matching credit into
 * store.journal_lines. Debits equal credits after every operation, always.
 */
(function (global) {
  'use strict';
