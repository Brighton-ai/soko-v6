...
async function main() {
  ...
  // Marks go in the class the pupil is actually IN. They used to be handed out
  // round-robin while pupils were placed by roster index, so a pupil in 4 East
  // had their marks recorded against 7 East — the mark sheet for a class did
  // not contain its own roll, and the report cards for a class ranked pupils
  // who were not in it.
  const byClass = {};
  students.forEach((st, i) => {
    const cls = classes.filter((c) => c.id === st.class_id)[0] || classes[i % classes.length];
  ...
  // Report cards are derived from marks and are regenerated by the suite. Left
  // behind, they carry the previous run's published state into the next one,
  // and rules about publishing find nothing left to publish.
  sql(`DELETE FROM report_cards WHERE tenant_id = '${tenantId}'`);
  ...
  //
  // A tenant made by SQL has no accounts, and the first payment against it
  // fails with "Account not found: 1200". Registration seeds them now; a
  // hand-made tenant has to be told to. Both tenants get the same treatment,
  // so the fixture matches what a school that signed up would have.
  console.log('\n── chart of accounts ──');
  for (const t of [tenantId, otherTenantId]) {
    const out = execFileSync(
      path.join(BACKEND_DIR, '.venv/bin/python'),
      ['scripts/seed_tenant_defaults.py', t],
      { encoding: 'utf8', cwd: BACKEND_DIR, env: { ...process.env, DATABASE_URL: DB } }
    );
    log('accounts', out.trim());
  }
